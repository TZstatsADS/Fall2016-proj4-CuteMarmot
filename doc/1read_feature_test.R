...
rm(list = ls())
#Read words' data.
setwd("/Users/kaishengwang/Downloads/TestSongFile100/test/")
files <- dir(".", recursive = TRUE, full.names = TRUE)

#source("http://bioconductor.org/biocLite.R")
#biocLite("rhdf5")
library(rhdf5)
h5ls("/Users/kaishengwang/Downloads/TestSongFile100/test/testsong1.h5")
#######
termana <- h5read("/Users/kaishengwang/Downloads/TestSongFile100/test/testsong1.h5", "/analysis")
ptm <- proc.time()
...
segments_loudness_max <- array(0, c(100, 2000))
i <- 1
for (file in files){
  dfanal <- h5read(file, "/analysis")
...
segments_loudness_max_time <- array(0, c(100, 2000))
i <- 1
for (file in files){
  dfanal <- h5read(file, "/analysis")
...
segments_pitches <- array(0, c(100, 12, 2000))
...
#segments_timbre
segments_timbre <- array(0, c(100, 12, 2000))
dfanal <- h5read(file[1], "/analysis")
